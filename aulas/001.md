...
Esse comando nós utilizamos para começar um projeto com o Git. Chamamos a pasta do projeto do repositório ou só repo. Pra usar ele só entrar na pasta do seu projeto e executar assim:
```
git init
```
...
Com o comando `git status` você fica sabendo do rolê dos arquivos e pastas e com o `git add` você guarda esse momento dos arquivos e pastas para em seguida realizar com o commit (não lembro o que commit vê abaixo aí).
...
Mas se você quer pegar apenas alguns arquivos, você pode deixar o
comando mais direto, dessa forma:
```
